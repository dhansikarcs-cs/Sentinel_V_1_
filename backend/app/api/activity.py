...
@router.get("")
def get_activity_feed(user: User = Depends(require_role("psychologist")), days: int = Query(7, ge=1, le=90), db: Session = Depends(get_db)):
    ...
    for p in patient_usernames:
        for j in db.query(JournalEntry).filter(JournalEntry.patient_username == p, JournalEntry.timestamp >= cutoff).all():
            events.append({
                "type": "journal",
                "patient": p,
    ...
    for b in db.query(Booking).filter(Booking.psychologist_username == user.username, Booking.created_at >= cutoff).all():
        events.append({
            "type": "booking",
