...
from app.repositories import PatientRepository, JournalRepository, FollowupRepository
...
def get_wellness(user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ring_data = db.query(RingSensorLog).filter(RingSensorLog.patient_username == user.username).order_by(RingSensorLog.logged_at.desc()).first()
    moods = db.query(MoodLog).filter(MoodLog.patient_username == user.username).order_by(MoodLog.timestamp.desc()).limit(7).all()
    journals_today = db.query(JournalEntry).filter(
        JournalEntry.patient_username == user.username,
        JournalEntry.timestamp.like(f"{datetime.now(timezone.utc).strftime('%Y-%m-%d')}%")
    ).count()
...
async def upload_consent(file: UploadFile = File(...), user: User = Depends(get_current_user), db: Session = Depends(get_db)):
...
def assign_psychologist(username: str, psych_username: str, user: User = Depends(require_role("psychologist")), db: Session = Depends(get_db)):
    repo = PatientRepository(db)
