...
def create_psych_journal(entry: PsychJournalCreate, user: User = Depends(require_role("psychologist")), db: Session = Depends(get_db)):
    ...
    db.commit()
    db.refresh(journal)
    log_audit("psych_journal_created", user=user.username, role=user.role, severity="INFO", status="success", resource=str(journal.id), db=db)
    return journal
...
def get_psych_journals(user: User = Depends(require_role("psychologist")), db: Session = Depends(get_db)):
    journals = db.query(PsychJournalEntry).filter(PsychJournalEntry.psychologist_username == user.username).order_by(PsychJournalEntry.timestamp.desc()).all()
    return journals
