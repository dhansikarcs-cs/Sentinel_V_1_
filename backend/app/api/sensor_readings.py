...
def create_sensor_reading(data: SensorReadingCreate, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
...
def get_patient_sensor_readings(username: str, user: User = Depends(require_role("psychologist")), db: Session = Depends(get_db)):
    return (
        db.query(SensorReading)
