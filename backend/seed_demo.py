...
ordered = [
    "risk_assessments", "ai_analyses", "emotion_results", "sensor_readings",
    "psych_journal_entries", "clinical_notes", "notifications",
    "triage_queue", "crisis_logs", "crisis_states", "followups",
    "bookings", "psych_availability", "ring_sensor_log", "mood_log",
    "journal_entries", "patient_profiles", "audit_logs", "event_store",
]
...
cel = User(
    username="cel", password_hash=hash_password("1234"),
    name="Dr. Celeste R." , role="psychologist", clinic_code="DEMO",
    onboarding_step=99, contact_info="cel@sentinel.demo",
    psych_trusted_contact="trusted-cel@sentinel.demo",
    encryption_salt=salt(), created_at=now.isoformat(),
)
alaya = User(
    username="alaya", password_hash=hash_password("4321"),
    name="Alaya", role="patient", clinic_code="DEMO",
    onboarding_step=99, contact_info="alaya@sentinel.demo",
    trusted_contact="mom@sentinel.demo", assigned_psych="cel",
    encryption_salt=salt(), created_at=now.isoformat(),
)
...
for i, j in enumerate(journals):
    ...
    db.add(JournalEntry(
        patient_username="alaya", raw_content=j["content"],
        summary=j["patient"], clinical_summary=j["clinical"],
        ai_source=("ollama" if i % 3 == 0 else "groq"),
        emotions=j["emotions"], emotion_probabilities=json.dumps(probs),
        timestamp=ts, created_at=ts, version=1,
    ))
db.commit()
...
# ── 4. MOODS ──────────────────────────────────────────────────
mood_labels = [
    ("okay", "\U0001f610"), ("anxious", "\U0001f62c"), ("good", "\U0001f60a"),
    ("tired", "\U0001f634"), ("okay", "\U0001f610"), ("great", "\U0001f929"),
    ("sad", "\U0001f641"), ("okay", "\U0001f610"), ("good", "\U0001f60a"),
    ("hopeful", "\U0001f60c"), ("good", "\U0001f60a"), ("great", "\U0001f929"),
    ("okay", "\U0001f610"), ("proud", "\U0001f4aa"),
]
...
for i in range(10):
    ...
    db.add(RingSensorLog(
        patient_username="alaya", device_id="ring_alaya",
        bpm=random.randint(58, 86), stress=random.randint(12, 60),
        sleep_hours=round(random.uniform(5.2, 8.6), 1),
        spo2=random.randint(95, 99), hrv=random.randint(24, 68),
        logged_at=ts,
    ))
db.commit()
...
bookings = [
    dict(date=days_from_now(-6), time="10:00", session_type="Follow-up", status="Completed",
         explanation="Review of weekly progress", contact="alaya@sentinel.demo"),
    dict(date=days_from_now(-1), time="10:00", session_type="Therapy", status="Approved",
         explanation="Continuing CBT work on anxiety", contact="alaya@sentinel.demo"),
    dict(date=days_from_now(3), time="11:00", session_type="Therapy", status="Pending",
         explanation="Follow-up after good week; want to plan next steps", contact="alaya@sentinel.demo"),
]
for b in bookings:
    db.add(Booking(
        patient_username="alaya", psychologist_username="cel",
        date=b["date"], time=b["time"], session_type=b["session_type"],
        status=b["status"], contact=b["contact"], explanation=b["explanation"],
...
# ── 7. FOLLOWUP TASKS ─────────────────────────────────────────
followups = [
    dict(title="Daily Mood Check", desc="Log your mood every morning before 10 AM.", status="active", days=-2),
    dict(title="Gratitude List", desc="Write down 3 things you are grateful for tonight.", status="active", days=-1),
    dict(title="Grounding Practice", desc="Try the 5-4-3-2-1 grounding exercise during a stressful moment.", status="active", days=0),
    dict(title="Morning Walk", desc="Walk by the lake for at least 15 minutes.", status="completed", days=-4, grade="Excellent"),
    dict(title="Meditation Session", desc="10 minutes of guided meditation, 3 times this week.", status="completed", days=-8, grade="Good"),
]
for f in followups:
    db.add(FollowupTask(
        id=str(uuid.uuid4()), patient_username="alaya", psychologist_username="cel",
        title=f["title"], description=f["desc"], status=f["status"],
        grade=f.get("grade", ""), assigned_at=(now + timedelta(days=f["days"])).isoformat(),
...
# ── 8. CLINICAL NOTES ─────────────────────────────────────────
clinical_notes = [
    dict(ts=days_from_now(-6), notes="Patient presented with mild anxiety symptoms. Reviewed weekly journal; noted improvement in routine adherence. Provided validation and encouraged continued behavioral activation.",
    ...
    dict(ts=days_from_now(-1), notes="Good session. Patient reported first panic-free week and completed gratitude list independently. Reinforced gains, discussed next-stage coping strategies.",
...
for n in clinical_notes:
    db.add(ClinicalNote(
        psychologist_username="cel", patient_username="alaya",
        raw_notes=n["notes"], ai_synthesis=n["synth"], timestamp=n["ts"],
    ))
db.commit()
...
psych_journals = [
    ("Alaya made real progress today. The supermarket episode shook her, but she turned it into motivation. Watch for avoidance behavior developing around grocery settings.",
     "Session reflection: notable resilience; plan graded exposure for next week.", "groq"),
    ("She booked her follow-up without prompting. That's a shift in treatment alliance — from compliance to ownership. Very encouraging.",
     "Marker: patient-initiated care. Transitioning to maintenance phase.", "ollama"),
]
for content, summary, source in psych_journals:
    db.add(PsychJournalEntry(
        psychologist_username="cel", raw_content=content, summary=summary,
        ai_source=source, emotions="reflective, encouraged",
...
for jr in journal_rows[1:4]:
    db.add(RiskAssessment(
        journal_id=jr.id, patient_username="alaya",
        risk_score=random.choice([15, 20, 10]), triggered=0,
        confidence=round(random.uniform(0.7, 0.9), 2),
        explanation="Somatic anxiety indicators present; no suicidality markers detected. Moderate monitor flag.",
        algorithm_version="1.0.0", created_at=jr.timestamp,
    ))
db.commit()
...
# ── 11. NOTIFICATIONS ─────────────────────────────────────────
notifications = [
    dict(title="Welcome to Sentinel", message="Hi Alaya, your account is ready. Start by logging your mood or writing a journal entry.", ntype="system", days=13),
    dict(title="Booking Approved", message="Dr. Cel accepted your therapy session for tomorrow at 10:00.", ntype="info", days=1),
    dict(title="Booking Pending", message="Your follow-up request for next week is waiting for approval.", ntype="reminder", days=0),
    dict(title="Journal Streak", message="You've been journaling regularly. Keep it up — consistency is progress.", ntype="system", days=2),
]
for n in notifications:
    db.add(Notification(
        patient_username="alaya", title=n["title"], message=n["message"],
        notification_type=n["ntype"], sent_at=(now - timedelta(days=n["days"])).isoformat(),
    ))
db.commit()
