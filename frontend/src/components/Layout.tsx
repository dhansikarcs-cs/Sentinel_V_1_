...
export default function Layout() {
  ...
  const bookingMsg = lastBooking ? ({
    Approved: { text: '✅ Booking Accepted! Your session has been confirmed.', color: '#2E8B57' },
    Rejected: { text: '❌ Booking Declined.', color: '#C7463B' },
    Cancelled: { text: '🔴 Booking Cancelled.', color: '#6E837A' },
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh' }}>
      <aside style={{
        ...
        background: '#E8F2F0', borderRight: '1px solid #D9E7E3',
        ...
        <Link to={role === 'Psychologist' ? '/triage' : '/dashboard'} style={{ textDecoration: 'none' }}>
          <div style={{ fontSize: '1.15rem', fontWeight: 700, color: '#17796E', marginBottom: '16px', padding: '0 4px' }}>
            Sentinel
          </div>
        </Link>

        <div style={{ fontSize: '0.75rem', color: '#6E837A', padding: '0 4px', marginBottom: '4px' }}>
          {role === 'Psychologist' ? '🧑‍⚕️' : '👤'} {user?.name} <span style={{ color: '#7C9188' }}>({role})</span>
        </div>

        <div style={{ fontSize: '0.75rem', color: '#6E837A', fontStyle: 'italic', padding: '6px 0 6px 12px', borderLeft: '2px solid #17796E', lineHeight: '1.5', margin: '8px 0' }}>
          "{QUOTES[quoteIdx]}"
        </div>

        <hr style={{ margin: '8px 0', borderColor: '#D9E7E3', opacity: 0.4 }} />

        <div style={{ fontSize: '0.6875rem', color: '#7C9188', fontWeight: 600, marginBottom: '4px', padding: '0 4px' }}>
          📊 System Status
        </div>
        <div style={{ fontSize: '0.75rem', padding: '0 4px', marginBottom: '4px' }}>
          <span style={{ color: crisisState?.active ? '#C7463B' : '#2E8B57' }}>
            {crisisState?.active ? '🔴 Crisis Active' : '🟢 Online'}
          </span>
        </div>
        <div style={{ fontSize: '0.6875rem', padding: '0 4px', color: aiStatus?.any_available ? '#2E8B57' : '#C7463B' }}>
        ...
        {isPsychRole ? (
          <>
            ...
            <div style={{ padding: '0 4px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.6rem', color: '#6E837A', marginBottom: '3px' }}>
                <span>Workload</span>
                <span>{pendingBookings + silentPatients} tasks</span>
              </div>
              <div style={{ height: '5px', background: '#DFECE8', borderRadius: '3px', overflow: 'hidden' }}>
                <div style={{
                  height: '100%', borderRadius: '3px',
                  width: `${Math.min(((pendingBookings + silentPatients) / Math.max(patientCount, 1)) * 100, 100)}%`,
                  background: pendingBookings + silentPatients > patientCount * 0.5 ? '#C7463B' : pendingBookings + silentPatients > 0 ? '#B7791A' : '#2E8B57',
                  transition: 'width 0.3s',
                }} />
              </div>
              <div style={{ fontSize: '0.6rem', color: '#6E837A', marginTop: '3px', display: 'flex', justifyContent: 'space-between' }}>
            ...
            <hr style={{ margin: '8px 0', borderColor: '#D9E7E3', opacity: 0.4 }} />
            <div style={{ fontSize: '0.6875rem', color: '#7C9188', fontWeight: 600, marginBottom: '4px', padding: '0 4px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <span>⚠️ High Risk</span>
              <span style={{ background: '#C7463B22', color: '#B5453D', fontSize: '0.55rem', padding: '1px 5px', borderRadius: '3px' }}>
            ...
            <div style={{ fontSize: '0.6875rem', padding: '0 4px', maxHeight: '110px', overflow: 'auto' }}>
              {triagePriorities.filter((p: any) => p.tier === 'crisis' || p.tier === 'high').length === 0 && silentPatients === 0 && !crisisState?.active ? (
                <div style={{ color: '#2E8B57', padding: '2px 0' }}>✅ All patients stable</div>
              ) : (
                <>
                  {crisisState?.active && (
                    <div style={{ color: '#C7463B', padding: '3px 0', borderBottom: '1px solid #D9E7E3', fontWeight: 600 }}>
                      🚨 {crisisState.patient || 'unknown'}
                    </div>
                  )}
                  {triagePriorities.filter((p: any) => p.tier === 'high').slice(0, 5).map((p: any) => (
                    <div key={p.patient} style={{ display: 'flex', justifyContent: 'space-between', padding: '3px 0', borderBottom: '1px solid #D9E7E3', color: '#B7791A', fontSize: '0.65rem' }}>
                      <span>{p.name}</span>
                      <span style={{ color: '#6E837A', fontSize: '0.6rem' }}>score {p.score}</span>
                    </div>
                  ))}
                  {silentPatients > 0 && (
                    <div style={{ color: '#B7791A', padding: '3px 0', fontSize: '0.65rem' }}>
            ...
            <div style={{ fontSize: '0.6875rem', padding: '0 4px', maxHeight: '100px', overflow: 'auto' }}>
              {Object.keys(aiInsights).length === 0 ? (
                <div style={{ color: '#A8B9B1' }}>Loading...</div>
              ) : (
                Object.entries(aiInsights).slice(0, 4).map(([patient, insight]: [string, any]) => (
                  <div key={patient} style={{ padding: '3px 0', borderBottom: '1px solid #D9E7E3' }}>
                    <div style={{ color: '#17796E', fontWeight: 600, fontSize: '0.625rem' }}>{patient}</div>
                    <div style={{ color: '#5F7A70', fontSize: '0.6rem', lineHeight: 1.4 }}>
          ...
          <>
            ...
            <button
              ...
              style={{ width: '100%', padding: '6px 8px', fontSize: '0.7rem', background: 'rgba(199,70,59,0.12)', border: '1px solid rgba(199,70,59,0.3)', borderRadius: '6px', color: '#B5453D', cursor: 'pointer', marginBottom: '4px' }}
            >
              🔥 Trigger SOS
            </button>
            {crisisState?.active && (
              <div style={{ fontSize: '0.65rem', color: '#C7463B', padding: '4px' }}>
                🚨 Crisis active
              </div>
            )}
            ...
            <div style={{ flex: 1, overflow: 'auto', fontSize: '0.6875rem', padding: '0 4px' }}>
              {journals.length === 0 ? (
                <div style={{ color: '#A8B9B1' }}>No entries yet.</div>
              ) : (
                journals.slice(0, 6).map((j: any, i: number) => (
                  <div key={i} style={{ display: 'flex', gap: '6px', padding: '3px 0', borderBottom: '1px solid #D9E7E3', alignItems: 'center' }}>
                    <span style={{ color: '#6E837A', fontSize: '0.625rem' }}>📝</span>
                    <span style={{ color: '#6E837A', fontSize: '0.625rem' }}>{(j.timestamp || '').slice(0, 10)}</span>
                    <span style={{ color: '#7C9188', fontSize: '0.625rem' }}>{(j.emotions || '').slice(0, 20) || 'journal entry'}</span>
                  </div>
                ))
              )}
            </div>
        ...
        <div style={{ fontSize: '0.6875rem', color: '#7C9188', fontWeight: 600, marginBottom: '4px', padding: '0 4px', display: 'flex', alignItems: 'center', gap: '6px' }}>
          🔔 Notifications
          {unreadCount > 0 && (
            <span style={{ background: '#C7463B', color: 'white', fontSize: '0.55rem', borderRadius: '50%', width: '16px', height: '16px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700 }}>
              {unreadCount}
            </span>
          )}
        </div>
        <div style={{ flex: 1, overflow: 'auto', fontSize: '0.6875rem', padding: '0 4px', maxHeight: '100px' }}>
          {notifications.filter((n: any) => !n.read).length === 0 ? (
            <div style={{ color: '#A8B9B1' }}>All caught up.</div>
          ) : (
            notifications.filter((n: any) => !n.read).slice(0, 4).map((n: any) => (
              <div key={n.id} style={{ padding: '3px 0', borderBottom: '1px solid #D9E7E3', cursor: 'pointer' }}
                onClick={() => { api.markNotificationRead(n.id).then(() => setNotifications(prev => prev.map((p: any) => p.id === n.id ? { ...p, read: true } : p))).catch(() => {}) }}>
                <span style={{ color: n.notification_type === 'crisis' ? '#C7463B' : '#17796E', fontWeight: 600 }}>{n.title}</span>
                <div style={{ color: '#6E837A', fontSize: '0.6rem' }}>{n.message?.slice(0, 60)}{n.message?.length > 60 ? '...' : ''}</div>
              </div>
            ))
          )}
        </div>
        ...
        {isPsychRole && crisisLog.length > 0 && (
          <>
            ...
            {crisisLog.slice(-3).reverse().map((e: any, i: number) => (
              <div key={i} style={{ fontSize: '0.6875rem', padding: '3px 0', color: '#7E948C', lineHeight: '1.5' }}>
                <strong>{e.event}</strong> <span style={{ color: '#A8B9B1', fontSize: '0.625rem' }}>{(e.timestamp || '').slice(0, 19).replace('T', ' ')}</span>
              </div>
            ))}
          </>
        )}

        <div style={{ marginTop: 'auto', paddingTop: '12px', borderTop: '1px solid #D9E7E3' }}>
          <button
            onClick={() => { logout(); navigate('/login') }}
            style={{
              width: '100%', padding: '8px 12px', fontSize: '0.75rem',
              background: '#FFFFFF', border: '1px solid #D9E7E3', borderRadius: '8px',
              color: '#7C9188', cursor: 'pointer', transition: 'all 0.2s',
            }}
            onMouseEnter={e => { e.currentTarget.style.background = '#17796E'; e.currentTarget.style.color = '#E8F2F0' }}
            onMouseLeave={e => { e.currentTarget.style.background = '#FFFFFF'; e.currentTarget.style.color = '#7C9188' }}
          >
            Logout
          </button>
        </div>
      ...
      <main style={{ flex: 1, padding: '28px 36px', overflow: 'auto' }}>
        <div style={{ maxWidth: '1100px' }}>
          {showCrisisBanner && (
            <div className="card" style={{ borderColor: '#C7463B', background: 'rgba(199,70,59,0.08)', marginBottom: '16px', padding: '16px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '4px' }}>
                <span style={{ color: '#C7463B', fontSize: '1.25rem' }}>⏱️ {crisisElapsed >= 60 ? '60+' : crisisElapsed}s</span>
                <span style={{ color: CRISIS_STAGE_MESSAGES[crisisStage]?.color || '#C7463B', fontWeight: 600, fontSize: '0.875rem' }}>
                ...
                {[
                  ...
                  return (
                    <div key={s.key} style={{
                      ...
                      background: isActive ? 'rgba(199,70,59,0.15)' : passed ? 'rgba(46,139,87,0.12)' : 'rgba(232,242,240,0.6)',
                      border: `1px solid ${isActive ? 'rgba(199,70,59,0.4)' : passed ? 'rgba(46,139,87,0.3)' : '#DFECE8'}`,
                      color: isActive ? '#C7463B' : passed ? '#2E8B57' : '#90A79F',
          ...
          {bookingMsg && (
            <div className="card" style={{ borderColor: '#B7791A', marginBottom: '12px', padding: '10px 14px' }}>
          ...
          <div style={{ marginBottom: '16px' }}>
            ...
            {role === 'Patient' && user?.assigned_psych && (
              <div style={{ color: '#7E948C', fontSize: '0.8125rem', marginTop: '4px' }}>
                💉 Your psychologist: <strong style={{ color: '#17796E' }}>{user.assigned_psych}</strong>
              </div>
            )}
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '8px', marginBottom: '16px' }}>
            {role === 'Psychologist' ? (
              <>
                <StatusCard label="Patients" value={patientCount} unit="under care" color="#17796E" />
                <StatusCard label="Pending" value={pendingBookings} unit="booking approvals" color={pendingBookings > 0 ? '#B7791A' : '#17796E'} />
                <StatusCard label="Silent" value={silentPatients} unit="patients >48h" color={silentPatients > 0 ? '#C7463B' : '#2E8B57'} />
                <StatusCard label="Crisis" value={crisisState?.active ? 'ACTIVE' : 'None'} unit="current" color={crisisState?.active ? '#C7463B' : '#6E837A'} />
              </>
            ) : (
              <>
                <StatusCard label="Heart" value={heartRate} unit="bpm" color="#CC5A4E" />
                <StatusCard label="Journal" value={journalOk} unit="today" color={journalOk === 'Logged' ? '#2E8B57' : '#6E837A'} />
                <StatusCard label="Next Session" value={nextSession} unit="" color={nextSession !== '-' ? '#2E8B57' : '#6E837A'} />
                <StatusCard label="Mood" value={todayMood} unit="today" color={todayMood !== '-' ? '#2E8B57' : '#6E837A'} />
              </>
            )}
          </div>
...
function StatusCard({ label, value, unit, color }: { label: string; value: string | number; unit: string; color: string }) {
  return (
    <div style={{ background: 'linear-gradient(135deg, #FFFFFF, #FFFFFF)', border: '1px solid #D9E7E3', borderRadius: '8px', padding: '8px 12px', display: 'flex', alignItems: 'center', gap: '12px' }}>
      <div>
        <div style={{ color: '#6E837A', fontSize: '0.6875rem', fontWeight: 500 }}>{label}</div>
        <div style={{ color, fontSize: '1.125rem', fontWeight: 700 }}>{value}</div>
      </div>
      {unit && <div style={{ color: '#A8B9B1', fontSize: '0.625rem' }}>{unit}</div>}
    </div>
  )
}
