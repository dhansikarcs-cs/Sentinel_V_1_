...
export default function Layout() {
  ...
  const bookingMsg = lastBooking ? ({
    ...
    Cancelled: { text: '🔴 Booking Cancelled.', color: '#7d877e' },
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh' }}>
      <aside style={{
        ...
        background: '#121715', borderRight: '1px solid #31423a',
        ...
        <Link to={role === 'Psychologist' ? '/triage' : '/dashboard'} style={{ textDecoration: 'none' }}>
          <div style={{ fontSize: '1.15rem', fontWeight: 700, color: '#8fcbb1', marginBottom: '16px', padding: '0 4px' }}>
            Sentinel
          </div>
        </Link>

        <div style={{ fontSize: '0.75rem', color: '#7d877e', padding: '0 4px', marginBottom: '4px' }}>
          {role === 'Psychologist' ? '🧑‍⚕️' : '👤'} {user?.name} <span style={{ color: '#9ca99e' }}>({role})</span>
        </div>

        <div style={{ fontSize: '0.75rem', color: '#7d877e', fontStyle: 'italic', padding: '6px 0 6px 12px', borderLeft: '2px solid #8fcbb1', lineHeight: '1.5', margin: '8px 0' }}>
          "{QUOTES[quoteIdx]}"
        </div>

        <hr style={{ margin: '8px 0', borderColor: '#31423a', opacity: 0.4 }} />

        <div style={{ fontSize: '0.6875rem', color: '#9ca99e', fontWeight: 600, marginBottom: '4px', padding: '0 4px' }}>
          📊 System Status
        </div>
        ...
        {isPsychRole ? (
          <>
            ...
            <div style={{ padding: '0 4px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.6rem', color: '#7d877e', marginBottom: '3px' }}>
                <span>Workload</span>
                <span>{pendingBookings + silentPatients} tasks</span>
              </div>
              <div style={{ height: '5px', background: '#223028', borderRadius: '3px', overflow: 'hidden' }}>
                <div style={{
              ...
              <div style={{ fontSize: '0.6rem', color: '#7d877e', marginTop: '3px', display: 'flex', justifyContent: 'space-between' }}>
            ...
            <hr style={{ margin: '8px 0', borderColor: '#31423a', opacity: 0.4 }} />
            <div style={{ fontSize: '0.6875rem', color: '#9ca99e', fontWeight: 600, marginBottom: '4px', padding: '0 4px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <div style={{ fontSize: '0.6875rem', padding: '0 4px', maxHeight: '110px', overflow: 'auto' }}>
              {triagePriorities.filter((p: any) => p.tier === 'crisis' || p.tier === 'high').length === 0 && silentPatients === 0 && !crisisState?.active ? (
                ...
                <>
                  {crisisState?.active && (
                    <div style={{ color: '#ef4444', padding: '3px 0', borderBottom: '1px solid #31423a', fontWeight: 600 }}>
                      🚨 {crisisState.patient || 'unknown'}
                    </div>
                  )}
                  {triagePriorities.filter((p: any) => p.tier === 'high').slice(0, 5).map((p: any) => (
                    <div key={p.patient} style={{ display: 'flex', justifyContent: 'space-between', padding: '3px 0', borderBottom: '1px solid #31423a', color: '#f59e0b', fontSize: '0.65rem' }}>
                      <span>{p.name}</span>
                      <span style={{ color: '#7d877e', fontSize: '0.6rem' }}>score {p.score}</span>
                    </div>
                  ))}
                  {silentPatients > 0 && (
            ...
            <div style={{ fontSize: '0.6875rem', padding: '0 4px', maxHeight: '100px', overflow: 'auto' }}>
              {Object.keys(aiInsights).length === 0 ? (
                <div style={{ color: '#6b766d' }}>Loading...</div>
              ) : (
                Object.entries(aiInsights).slice(0, 4).map(([patient, insight]: [string, any]) => (
                  <div key={patient} style={{ padding: '3px 0', borderBottom: '1px solid #31423a' }}>
                    <div style={{ color: '#8fcbb1', fontWeight: 600, fontSize: '0.625rem' }}>{patient}</div>
                    <div style={{ color: '#8aa198', fontSize: '0.6rem', lineHeight: 1.4 }}>
          ...
          <>
            ...
            <div style={{ flex: 1, overflow: 'auto', fontSize: '0.6875rem', padding: '0 4px' }}>
              {journals.length === 0 ? (
                <div style={{ color: '#6b766d' }}>No entries yet.</div>
              ) : (
                journals.slice(0, 6).map((j: any, i: number) => (
                  <div key={i} style={{ display: 'flex', gap: '6px', padding: '3px 0', borderBottom: '1px solid #31423a', alignItems: 'center' }}>
                    <span style={{ color: '#7d877e', fontSize: '0.625rem' }}>📝</span>
                    <span style={{ color: '#7d877e', fontSize: '0.625rem' }}>{(j.timestamp || '').slice(0, 10)}</span>
                    <span style={{ color: '#9ca99e', fontSize: '0.625rem' }}>{(j.emotions || '').slice(0, 20) || 'journal entry'}</span>
                  </div>
                ))
              )}
            </div>
        ...
        <div style={{ fontSize: '0.6875rem', color: '#9ca99e', fontWeight: 600, marginBottom: '4px', padding: '0 4px', display: 'flex', alignItems: 'center', gap: '6px' }}>
          🔔 Notifications
          {unreadCount > 0 && (
        ...
        <div style={{ flex: 1, overflow: 'auto', fontSize: '0.6875rem', padding: '0 4px', maxHeight: '100px' }}>
          {notifications.filter((n: any) => !n.read).length === 0 ? (
            <div style={{ color: '#6b766d' }}>All caught up.</div>
          ) : (
            notifications.filter((n: any) => !n.read).slice(0, 4).map((n: any) => (
              <div key={n.id} style={{ padding: '3px 0', borderBottom: '1px solid #31423a', cursor: 'pointer' }}
                onClick={() => { api.markNotificationRead(n.id).then(() => setNotifications(prev => prev.map((p: any) => p.id === n.id ? { ...p, read: true } : p))).catch(() => {}) }}>
                <span style={{ color: n.notification_type === 'crisis' ? '#ef4444' : '#8fcbb1', fontWeight: 600 }}>{n.title}</span>
                <div style={{ color: '#7d877e', fontSize: '0.6rem' }}>{n.message?.slice(0, 60)}{n.message?.length > 60 ? '...' : ''}</div>
              </div>
            ))
          )}
        </div>
        ...
        {isPsychRole && crisisLog.length > 0 && (
          <>
            ...
            {crisisLog.slice(-3).reverse().map((e: any, i: number) => (
              <div key={i} style={{ fontSize: '0.6875rem', padding: '3px 0', color: '#bac9bf', lineHeight: '1.5' }}>
                <strong>{e.event}</strong> <span style={{ color: '#6b766d', fontSize: '0.625rem' }}>{(e.timestamp || '').slice(0, 19).replace('T', ' ')}</span>
              </div>
            ))}
          </>
        )}

        <div style={{ marginTop: 'auto', paddingTop: '12px', borderTop: '1px solid #31423a' }}>
          <button
            onClick={() => { logout(); navigate('/login') }}
            style={{
              width: '100%', padding: '8px 12px', fontSize: '0.75rem',
              background: '#1d2623', border: '1px solid #31423a', borderRadius: '8px',
              color: '#9ca99e', cursor: 'pointer', transition: 'all 0.2s',
            }}
            onMouseEnter={e => { e.currentTarget.style.background = '#8fcbb1'; e.currentTarget.style.color = '#121715' }}
            onMouseLeave={e => { e.currentTarget.style.background = '#1d2623'; e.currentTarget.style.color = '#9ca99e' }}
          >
            Logout
          </button>
        </div>
        ...
        <div style={{ maxWidth: '1100px' }}>
                ...
                {[
                  ...
                  return (
                    <div key={s.key} style={{
                      ...
                      background: isActive ? 'rgba(239,68,68,0.15)' : passed ? 'rgba(34,197,94,0.12)' : 'rgba(28,40,33,0.6)',
                      border: `1px solid ${isActive ? 'rgba(239,68,68,0.4)' : passed ? 'rgba(34,197,94,0.3)' : '#223028'}`,
                      color: isActive ? '#ef4444' : passed ? '#22c55e' : '#3d4d45',
          ...
          <div style={{ marginBottom: '16px' }}>
            ...
            {role === 'Patient' && user?.assigned_psych && (
              <div style={{ color: '#bac9bf', fontSize: '0.8125rem', marginTop: '4px' }}>
                💉 Your psychologist: <strong style={{ color: '#8fcbb1' }}>{user.assigned_psych}</strong>
              </div>
            )}
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '8px', marginBottom: '16px' }}>
            {role === 'Psychologist' ? (
              <>
                <StatusCard label="Patients" value={patientCount} unit="under care" color="#8fcbb1" />
                <StatusCard label="Pending" value={pendingBookings} unit="booking approvals" color={pendingBookings > 0 ? '#f59e0b' : '#8fcbb1'} />
                <StatusCard label="Silent" value={silentPatients} unit="patients >48h" color={silentPatients > 0 ? '#ef4444' : '#22c55e'} />
                <StatusCard label="Crisis" value={crisisState?.active ? 'ACTIVE' : 'None'} unit="current" color={crisisState?.active ? '#ef4444' : '#7d877e'} />
              </>
            ) : (
              <>
                <StatusCard label="Heart" value={heartRate} unit="bpm" color="#ff6b6b" />
                <StatusCard label="Journal" value={journalOk} unit="today" color={journalOk === 'Logged' ? '#22c55e' : '#7d877e'} />
                <StatusCard label="Next Session" value={nextSession} unit="" color={nextSession !== '-' ? '#22c55e' : '#7d877e'} />
                <StatusCard label="Mood" value={todayMood} unit="today" color={todayMood !== '-' ? '#22c55e' : '#7d877e'} />
              </>
            )}
          </div>
...
function StatusCard({ label, value, unit, color }: { label: string; value: string | number; unit: string; color: string }) {
  return (
    <div style={{ background: 'linear-gradient(135deg, #1d2623, #19211e)', border: '1px solid #31423a', borderRadius: '8px', padding: '8px 12px', display: 'flex', alignItems: 'center', gap: '12px' }}>
      <div>
        <div style={{ color: '#7d877e', fontSize: '0.6875rem', fontWeight: 500 }}>{label}</div>
        <div style={{ color, fontSize: '1.125rem', fontWeight: 700 }}>{value}</div>
      </div>
      {unit && <div style={{ color: '#6b766d', fontSize: '0.625rem' }}>{unit}</div>}
    </div>
  )
}
