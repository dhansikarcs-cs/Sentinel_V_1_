      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '24px', alignItems: 'start' }}>
        <div className="space-y-4">
                ...
                {aiDraft.themes && (
                  <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap', marginTop: '8px' }}>
                    {aiDraft.themes.map((t: string) => (
                      <span key={t} style={{ background: 'var(--border)', color: 'var(--secondary)', fontSize: '0.6875rem', padding: '2px 8px', borderRadius: '4px' }}>{t}</span>
                    ))}
                  </div>
                )}
          ...
          <div className="card" style={{ padding: '20px' }}>
            ...
            {notes.length === 0 ? (
              <p style={{ color: 'var(--muted)', fontSize: '0.875rem' }}>No notes yet.</p>
            ) : (
                ...
                {notes.slice(0, 10).map((n: any) => (
                  <div key={n.id} className="card-stage" style={{ flexDirection: 'column', alignItems: 'stretch' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
                      <span style={{ fontSize: '0.8125rem', color: 'var(--accent)', fontWeight: 600 }}>{n.patient}</span>
                      <span style={{ fontSize: '0.6875rem', color: 'var(--muted)' }}>{n.timestamp?.slice(0, 10)}</span>
                    </div>
                    <div style={{ fontSize: '0.75rem', color: 'var(--secondary)', lineHeight: 1.5 }}>{n.ai_synthesis?.slice(0, 200)}</div>
                  </div>
                ))}
              </div>
            )}
        ...
        <div className="card" style={{ padding: '20px' }}>
          ...
          <p style={{ fontSize: '0.75rem', color: 'var(--muted)', marginBottom: '12px', lineHeight: 1.6 }}>
          ...
          {!j2n && (
            <div className="space-y-2">
              {patients.length === 0 && <p style={{ color: 'var(--muted)', fontSize: '0.8125rem' }}>No patients available.</p>}
              {patients.slice(0, 5).map((p: any) => (
                <button
                  ...
                  style={{
                    ...
                    background: 'var(--surface)', border: '1px solid var(--border)', color: 'var(--text)', fontSize: '0.8125rem',
          ...
          {j2n && j2n.status === 'empty' && (
            <div>
              <div style={{ color: 'var(--secondary)', fontSize: '0.8125rem', marginBottom: '12px' }}>
          ...
          {j2n && j2n.status === 'error' && (
            <div>
              <div style={{ color: 'var(--danger-deep)', fontSize: '0.8125rem', marginBottom: '12px' }}>
          ...
          {j2n && j2n.status === 'draft' && (
            <div>
              <div style={{ fontSize: '0.6875rem', color: 'var(--muted)', marginBottom: '8px' }}>
              ...
              <div style={{
                ...
                background: 'var(--surface-soft)', border: '1px solid var(--border)', color: 'var(--text)', maxHeight: '240px', overflow: 'auto',
              }}>
