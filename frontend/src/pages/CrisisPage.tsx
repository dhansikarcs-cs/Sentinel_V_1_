...
export default function CrisisPage() {
  ...
  if (loading) return <div className="animate-fade-in"><h2>🚨 Emergency</h2><div className="card"><span style={{ color: 'var(--muted)' }}>Loading...</span></div></div>
  ...
  function stageStyle(key: string) {
    ...
    if (isActive) return { color: 'var(--danger)', background: 'rgba(199,70,59,0.15)', border: '1px solid rgba(199,70,59,0.4)' }
    if (passed) return { color: 'var(--ok)', background: 'var(--ok-alpha)', border: '1px solid rgba(46,139,87,0.3)' }
    ...
    <div className="animate-fade-in">
      ...
      {!active ? (
        <div style={{ display: 'flex', gap: '12px', marginTop: '16px' }}>
          {isPsych ? (
            <button onClick={trigger} className="btn-primary" style={{ flex: 1, padding: '14px', fontSize: '1rem', background: 'linear-gradient(135deg, var(--danger), #dc2626)' }}>
        ...
        <div style={{ marginTop: '16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
          {error && (
            <div style={{ background: 'rgba(199,70,59,0.1)', border: '1px solid rgba(199,70,59,0.3)', borderRadius: '8px', padding: '10px 14px', color: 'var(--danger-deep)', fontSize: '0.8125rem' }}>
              {error}
            </div>
          )}
          {triggeredBy === 'psychologist' && (
            <div className="card" style={{ borderColor: 'var(--danger)', color: 'var(--danger-deep)' }}>
          ...
          {['trustee_coming', 'trustee_clicked'].includes(stage) && (
            <div style={{ color: 'var(--warn)' }}><strong>{CRISIS_STAGE_MESSAGES[stage].text}</strong></div>
          )}
          ...
          <div className="card-dark" style={{ padding: '12px', borderColor: 'var(--border-soft)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
              <span style={{ color: 'var(--danger-deep)', fontSize: '1.125rem' }}>⏱️ Timer</span>
              <span style={{ color: 'var(--strong)', fontSize: '1.25rem', fontWeight: 700 }}>{displayTime}s</span>
              <span style={{ color: 'var(--soft)', fontSize: '0.8125rem' }}>elapsed</span>
              <div style={{ marginLeft: 'auto' }}>
                {stage === 'helpline_escalated' && <span style={{ color: 'var(--danger)', fontWeight: 600, fontSize: '0.8125rem' }}>Helpline contacted</span>}
          ...
          {triggeredBy === 'patient' && !terminal && (
            <button onClick={handleCancel} disabled={cancelling} className="btn-primary" style={{ padding: '12px', background: 'var(--danger-deep)', border: '1px solid var(--danger)' }}>
              {cancelling ? 'Cancelling...' : '✅ Cancel Crisis'}
            </button>
          )}
          {isPsych && !terminal && (
            <button onClick={handleResolve} disabled={cancelling} className="btn-primary" style={{ padding: '12px', background: 'var(--ok)', border: '1px solid var(--ok)' }}>
              {cancelling ? 'Resolving...' : '🗑 Resolve Crisis'}
            </button>
          )}
          {triggeredBy === 'psychologist' && !isPsych && (
            <div style={{ color: 'var(--muted)', fontSize: '0.8125rem' }}>⚠️ This crisis was triggered by your psychologist. Only they can resolve it.</div>
          )}
