  ...
  return (
    <div className="animate-fade-in space-y-4">
      ...
      <div style={{ display: 'flex', gap: '8px', alignItems: 'center', marginBottom: '4px' }}>
        ...
        <span style={{ color: '#6b766d', fontSize: '0.6875rem' }}>24-hour trend data</span>
      </div>
      ...
      {showTable && (
        <div className="card" style={{ padding: '16px', overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.75rem' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid #31423a' }}>
                <th style={{ padding: '6px 8px', textAlign: 'left', color: '#9ca99e' }}>#</th>
                {trends.map(t => <th key={t.key} style={{ padding: '6px 8px', textAlign: 'right', color: t.color }}>{t.label}</th>)}
              </tr>
            </thead>
            <tbody>
              {trends[0].data.slice(-24).map((_, i) => (
                <tr key={i} style={{ borderBottom: '1px solid #1d2623' }}>
                  <td style={{ padding: '4px 8px', color: '#7d877e' }}>{i + 1}</td>
                  {trends.map(t => (
                    <td key={t.key} style={{ padding: '4px 8px', textAlign: 'right', color: '#d9ddd3' }}>{t.data[t.data.length - 24 + i]?.v ?? '-'}</td>
                  ))}
                </tr>
              ))}
            </tbody>
      ...
      <div className="card" style={{ padding: '20px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px', flexWrap: 'wrap' }}>
          <div>
            ...
            <div style={{ fontSize: '0.8125rem', color: '#9ca99e' }}>{wellness?.mood?.label || 'Not logged'}</div>
          </div>
          <div style={{ width: '1px', height: '40px', background: '#31423a' }} />
          <div>
            <div style={{ fontSize: '0.75rem', color: '#7d877e' }}>Journal Activity</div>
      ...
      {wellness?.mood_trend && wellness.mood_trend.length > 0 && (
        <div className="card" style={{ padding: '20px' }}>
          <div style={{ fontSize: '0.8125rem', color: '#9ca99e', fontWeight: 600, marginBottom: '12px' }}>Mood Trend (7 days)</div>
          <div style={{ display: 'flex', gap: '8px', justifyContent: 'center', flexWrap: 'wrap' }}>
            {wellness.mood_trend.map((m: any, i: number) => (
              <div key={i} style={{ textAlign: 'center', padding: '8px', background: '#1d2623', borderRadius: '8px', minWidth: '60px' }}>
                <div style={{ fontSize: '1.5rem' }}>{m.emoji}</div>
                <div style={{ fontSize: '0.65rem', color: '#7d877e', marginTop: '2px' }}>{m.date?.slice(-2)}</div>
              </div>
            ))}
          </div>
        </div>
            ...
            <div className="expander-body">
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '12px', marginBottom: '12px' }}>
                <div className="metric-card" style={{ textAlign: 'center', padding: '12px' }}>
                  <div style={{ color: '#9ca99e', fontSize: '0.75rem' }}>📝 Journals (7d)</div>
                  <div style={{ color: '#e7e9df', fontSize: '1.5rem', fontWeight: 700 }}>{wellness.ai_insights.journal_count || 0}</div>
                </div>
                <div className="metric-card" style={{ textAlign: 'center', padding: '12px' }}>
                  <div style={{ color: '#9ca99e', fontSize: '0.75rem' }}>✅ Compliance</div>
                  <div style={{ color: '#e7e9df', fontSize: '1.5rem', fontWeight: 700 }}>{wellness.ai_insights.compliance || 0}%</div>
                </div>
                <div className="metric-card" style={{ textAlign: 'center', padding: '12px' }}>
                  <div style={{ color: '#9ca99e', fontSize: '0.75rem' }}>❌ Missed</div>
                  <div style={{ color: '#e7e9df', fontSize: '1.5rem', fontWeight: 700 }}>{wellness.ai_insights.missed || 0}</div>
                </div>
              </div>
              {wellness.ai_insights.grades && (
                <div style={{ color: '#7d877e', fontSize: '0.75rem', marginBottom: '8px' }}>
              ...
              {wellness.ai_insights.mood_message && (
                <div style={{ color: '#9ca99e', fontSize: '0.8125rem', marginBottom: '4px' }}>{wellness.ai_insights.mood_message}</div>
              )}
              <div style={{ marginTop: '6px', color: '#7d877e', fontSize: '0.65rem', lineHeight: 1.5 }}>
