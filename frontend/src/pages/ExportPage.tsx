    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="card" style={{ padding: '16px' }}>
        <div style={{ display: 'flex', gap: '8px', marginBottom: '16px' }}>
          {['patients', 'myself'].map(m => (
            <button key={m} onClick={() => setMode(m as any)}
              style={{
                flex: 1, padding: '10px', borderRadius: '8px', border: `1px solid ${mode === m ? '#c49ea4' : '#2d2d44'}`,
                background: mode === m ? '#232840' : '#1e2336', color: mode === m ? '#c49ea4' : '#d8d4dc',
        ...
        <div style={{ display: 'flex', gap: '12px', marginBottom: '16px', alignItems: 'center' }}>
          <span style={{ color: '#6a6474', fontSize: '0.8125rem' }}>📅 Filter by date:</span>
          <input type="date" value={dateFrom} onChange={e => setDateFrom(e.target.value)}
            style={{ padding: '6px 10px', fontSize: '0.8rem', background: '#1e2336', border: '1px solid #2d2d44', borderRadius: '6px', color: '#c0d0e0' }} />
          <span style={{ color: '#3a4a5a', fontSize: '0.75rem' }}>to</span>
          <input type="date" value={dateTo} onChange={e => setDateTo(e.target.value)}
            style={{ padding: '6px 10px', fontSize: '0.8rem', background: '#1e2336', border: '1px solid #2d2d44', borderRadius: '6px', color: '#c0d0e0' }} />
          {(dateFrom || dateTo) && (
            <button onClick={() => { setDateFrom(''); setDateTo('') }}
              style={{ padding: '6px 12px', fontSize: '0.75rem', background: '#1e2336', border: '1px solid #2d2d44', borderRadius: '6px', color: '#6a6474', cursor: 'pointer' }}>
              Clear
            </button>
          )}
        </div>
        ...
        {mode === 'patients' ? (
          <>
            {patients.length === 0 ? (
              <div style={{ color: '#6a6474', fontSize: '0.8125rem' }}>No patients assigned.</div>
            ) : (
              <>
                <div style={{ fontSize: '0.8125rem', color: '#9a92a2', fontWeight: 600, marginBottom: '8px' }}>Select a patient</div>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginBottom: '16px' }}>
                  {patients.map((p: any) => (
                    <button key={p.username || p} onClick={() => setSelectedPatient(p.username || p)}
                      style={{
                        padding: '8px 16px', borderRadius: '8px', border: `1px solid ${selectedPatient === (p.username || p) ? '#c49ea4' : '#2d2d44'}`,
                        background: selectedPatient === (p.username || p) ? '#232840' : '#1e2336',
                        color: selectedPatient === (p.username || p) ? '#c49ea4' : '#d8d4dc', fontSize: '0.8125rem', cursor: 'pointer',
                      }}>
                      {p.name || p.username || p}
                    </button>
                  ))}
                ...
                {selectedPatient && (
                  <>
                    <h3 style={{ fontSize: '0.9rem', margin: '0 0 8px 0', color: '#c49ea4' }}>
                      {patients.find((p: any) => (p.username || p) === selectedPatient)?.name || selectedPatient}
                    </h3>

                    <div style={{ fontSize: '0.8125rem', color: '#9a92a2', fontWeight: 600, marginBottom: '8px' }}>Journal Entries {dateFrom || dateTo ? `(${filteredEntries.length} shown)` : ''}</div>
                    {filteredEntries.length === 0 ? (
                      <div style={{ color: '#6a6474', fontSize: '0.8125rem', marginBottom: '16px' }}>No journal entries.</div>
                    ) : (
                          ...
                          <div key={key} style={{ marginBottom: '6px' }}>
                            <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                              <button onClick={() => toggleExpand(setExpandedEntries, key)}
                                style={{
                                  flex: 1, padding: '6px 10px', background: '#1e2336',
                                  border: '1px solid #2d2d44', borderRadius: '6px',
                              ...
                              <button onClick={() => downloadCsv(`${selectedPatient}_journal_${i}.csv`, [['Timestamp', 'Summary'], [ts, e.summary || '']])}
                                style={{ padding: '6px 10px', background: '#1e2336', border: '1px solid #2d2d44', borderRadius: '6px', color: '#9a92a2', cursor: 'pointer', fontSize: '0.75rem' }}>
                                ⬇
                              </button>
                            </div>
                            {open && (
                              <div style={{ background: '#1a2238', border: '1px solid #1e3a5a', borderRadius: '8px', padding: '12px', margin: '4px 0 0 0' }}>
                                ...
                                {e.emotions && <div style={{ color: '#6a6474', fontSize: '0.6875rem', marginTop: '4px' }}>Emotions: {e.emotions}</div>}
                              </div>
                            )}
                          </div>
                        )
                    ...
                    {filteredNotes.length > 0 && (
                      <>
                        <div style={{ fontSize: '0.8125rem', color: '#9a92a2', fontWeight: 600, margin: '12px 0 8px 0' }}>Clinical Notes {dateFrom || dateTo ? `(${filteredNotes.length} shown)` : ''}</div>
                        {filteredNotes.map((n: any, i: number) => {
                          ...
                          return (
                            <div key={key} style={{ marginBottom: '6px' }}>
                              ...
                              {open && (
                                <div style={{ background: '#1a2238', border: '1px solid #1e3a5a', borderRadius: '8px', padding: '12px', margin: '4px 0 0 0' }}>
                                  <div style={{ color: '#d8d4dc', fontSize: '0.8125rem', lineHeight: 1.6 }}>{n.ai_synthesis || n.raw_notes}</div>
                                </div>
                              )}
                            </div>
                          )
          ...
          <div>
            <div style={{ fontSize: '0.8125rem', color: '#9a92a2', fontWeight: 600, marginBottom: '12px' }}>My Journal Entries {dateFrom || dateTo ? `(${filteredOwn.length} shown)` : ''}</div>
            {filteredOwn.length === 0 ? (
              <div style={{ color: '#6a6474', fontSize: '0.8125rem' }}>No journal entries yet.</div>
            ) : (
