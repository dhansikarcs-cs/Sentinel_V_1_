...
const STATUS_BADGE: Record<string, { label: string; color: string; bg: string }> = {
  pending: { label: '⏳ Pending', color: '#A66E0C', bg: 'rgba(251,191,36,0.12)' },
  completed: { label: '✅ Completed', color: 'var(--ok)', bg: 'var(--ok-alpha)' },
  skipped: { label: '❌ Skipped', color: 'var(--danger)', bg: 'var(--danger-alpha)' },
}

function dueInfo(due?: string): { text: string; color: string } | null {
  if (!due) return null
  const today = todayStr()
  if (due < today) return { text: `Overdue · ${due}`, color: 'var(--danger)' }
  if (due === today) return { text: `Due today · ${due}`, color: '#A66E0C' }
  return { text: `Due ${due}`, color: 'var(--soft)' }
}
...
function PatientFollowups() {
  ...
  if (myTasks.length === 0) return (
    <div className="animate-fade-in">
      ...
      <div className="card"><span style={{ color: 'var(--muted)', fontSize: '0.875rem' }}>No tasks assigned yet.</span></div>
    </div>
  )
    ...
    <div className="animate-fade-in">
      ...
      <div className="space-y-3">
        {myTasks.slice().reverse().map((t: any) => {
          ...
          return (
            <div key={t.id} style={{ border: '1px solid var(--border)', borderRadius: '10px', padding: '14px', background: 'var(--surface-soft)', marginBottom: '10px' }}>
              ...
              {t.description && <div style={{ color: 'var(--soft)', fontSize: '0.8125rem', marginBottom: '8px' }}>{t.description}</div>}
              ...
              {t.status === 'pending' && (
                <div>
                  <div style={{ display: 'flex', gap: '6px', alignItems: 'center', marginTop: '8px', flexWrap: 'wrap' }}>
                    <label style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '0.7rem', color: 'var(--secondary)', flex: 1, minWidth: '140px' }}>
                      <span>📎</span>
                      <input type="file" onChange={e => setUploadingProof({ ...uploadingProof, [t.id]: e.target.files?.[0] || null })}
                        style={{ fontSize: '0.7rem', color: 'var(--secondary)' }} />
                    </label>
              ...
              {t.status === 'completed' && (
                <div>
                  {t.grade && t.grade !== 'none' ? (
                    <div style={{ color: t.grade === 'green' ? '#44ff44' : t.grade === 'yellow' ? '#ffd93d' : 'var(--danger)', fontWeight: 'bold', fontSize: '14px' }}>
                      {t.grade === 'green' ? '🟢 Correctly done' : t.grade === 'yellow' ? '🟡 Partially done' : '🔴 Needs improvement'}
                    </div>
                  ) : (
                    <div style={{ color: 'var(--ok)', fontSize: '0.8125rem', fontWeight: 600 }}>✅ Submitted — awaiting review</div>
                  )}
                  {t.feedback ? (
                    <div style={{ marginTop: '8px', padding: '10px 12px', borderRadius: '8px', background: 'rgba(23,121,110,0.08)', border: '1px solid rgba(23,121,110,0.25)' }}>
                      <div style={{ color: 'var(--accent)', fontSize: '0.65rem', fontWeight: 600, marginBottom: '4px' }}>💬 Feedback from your psychologist</div>
              ...
              {t.status === 'skipped' && <div style={{ color: 'var(--danger)' }}>❌ Not completed</div>}

              <div style={{ color: 'var(--muted)', fontSize: '0.6875rem', marginTop: '8px' }}>Assigned: {t.assigned_at?.slice(0, 10)}</div>
            </div>
          )
        })}
      </div>
            ...
            <div className="space-y-2">
              {myTasks.slice().reverse().map((t: any) => {
                ...
                const borderColor = t.status === 'completed' && t.grade && t.grade !== 'none'
                  ? gradeBorders[t.grade] || 'var(--border)' : 'var(--border)'
                const due = dueInfo(t.due_date)
                return (
                  <div key={t.id} className="expander" style={{ borderColor }}>
                    ...
                    {open && (
                      <div className="expander-body">
                        ...
                        <div style={{ color: 'var(--soft)', fontSize: '0.8125rem', marginBottom: '8px' }}>{t.description}</div>
                        {due && <div style={{ fontSize: '0.6875rem', color: due.color, marginBottom: '8px' }}>{due.text}</div>}
                        ...
                        <div style={{ color: 'var(--muted)', fontSize: '0.6875rem', marginTop: '8px' }}>Assigned: {t.assigned_at?.slice(0, 10)}{t.due_date ? ` · Due: ${t.due_date}` : ''}</div>
                      </div>
                    )}
                  </div>
                )
            ...
            <div className="ai-box" style={{ marginTop: '8px' }}>
              ...
              {agentResult.tasks?.map((task: any, i: number) => (
                <div key={i} className="card" style={{ margin: '6px 0', padding: '10px' }}>
                  <div style={{ color: 'var(--accent)', fontSize: '0.8125rem', fontWeight: 600 }}>{task.title}</div>
                  <div style={{ color: 'var(--secondary)', fontSize: '0.75rem', marginTop: '4px' }}>{task.description}</div>
                  {assignedId === i ? (
                    <div style={{ color: 'var(--ok)', fontSize: '0.7rem', fontWeight: 600, marginTop: '6px' }}>✅ Assigned to {aiPatient}</div>
                  ) : (
