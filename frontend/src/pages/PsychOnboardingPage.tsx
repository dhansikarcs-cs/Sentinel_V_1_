    ...
    <div className="min-h-screen flex items-center justify-center" style={{ padding: '32px' }}>
      <div style={{ width: '100%', maxWidth: '560px' }}>
        <div style={{ textAlign: 'center', marginBottom: '24px' }}>
          ...
          <div style={{ color: '#6a6474', fontSize: '0.85rem', marginTop: '4px' }}>Set up your psychologist workspace</div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '8px', marginBottom: '24px' }}>
          {STEPS.map((s, i) => {
            const done = i < step
            const active = i === step
            const bg = done ? '#1a3a2a' : active ? '#2a2040' : '#1e2336'
            const border = done ? '#22c55e40' : active ? '#c49ea460' : '#2d2d44'
            const color = done ? '#22c55e' : active ? '#c49ea4' : '#5a4a5a'
        ...
        <div className="card" style={{ padding: '28px' }}>
          {step === 0 && (
            <div>
              <h3>🏠 Your Profile</h3>
              <div style={{ background: 'linear-gradient(135deg,#1e2336,#1e2a45)', border: '1px solid #2d2d44', borderRadius: '12px', padding: '20px', margin: '12px 0' }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                  {[
                    ...
                    <div key={f.label}>
                      <div style={{ color: '#6a6474', fontSize: '0.75rem' }}>{f.label}</div>
                      <div style={{ color: '#c0d0e0', fontSize: '1rem', fontWeight: 600, marginTop: '2px' }}>{f.value}</div>
                    </div>
                  ))}
                </div>
              </div>
              <p style={{ color: '#6a6474', fontSize: '0.8rem', marginBottom: '12px' }}>
          ...
          {step === 3 && (
            <div>
              <h3>🔮 Quick Tips</h3>
              <div style={{ background: '#1e2336', border: '1px solid #2d2d44', borderRadius: '10px', padding: '16px', margin: '12px 0' }}>
                {[
                  ...
                  <div key={i} style={{ display: 'flex', gap: '12px', marginBottom: i < 2 ? '10px' : 0 }}>
                    <span style={{ color: '#c49ea4', fontSize: '1.2rem' }}>{tip.emoji}</span>
                    <div>
                      <div style={{ color: '#c0d0e0', fontWeight: 600, fontSize: '0.875rem' }}>{tip.title}</div>
                      <div style={{ color: '#6a6474', fontSize: '0.75rem' }}>{tip.desc}</div>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {step >= 4 && (
            <div style={{ background: 'linear-gradient(135deg,#1a1e30,#1a3a2a)', border: '1px solid rgba(34,197,94,0.25)', borderRadius: '16px', padding: '32px', textAlign: 'center' }}>
              <div style={{ fontSize: '3rem', marginBottom: '8px' }}>✅</div>
              <div style={{ color: '#22c55e', fontSize: '1.5rem', fontWeight: 700 }}>You're all set!</div>
              <div style={{ color: '#6a6474', fontSize: '0.85rem', marginTop: '8px', lineHeight: 1.6 }}>
...
function ConsentUpload({ user }: { user: any }) {
  ...
  return (
    <div style={{ marginTop: '16px', padding: '12px', background: '#1a2238', border: '1px solid #1e2940', borderRadius: '8px', textAlign: 'left' }}>
      <div style={{ color: '#9a92a2', fontSize: '0.8rem', fontWeight: 600, marginBottom: '8px' }}>📄 Consent Form</div>
      {uploaded ? (
        <div style={{ color: '#22c55e', fontSize: '0.8125rem' }}>✅ Consent form uploaded. <button onClick={() => setUploaded('')} style={{ background: 'none', border: 'none', color: '#c49ea4', cursor: 'pointer', fontSize: '0.75rem', textDecoration: 'underline' }}>Re-upload</button></div>
      ) : (
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
          <input type="file" onChange={e => setFile(e.target.files?.[0] || null)} accept=".pdf,.jpg,.png"
            style={{ fontSize: '0.75rem', color: '#9a92a2', flex: 1 }} />
          <button onClick={handleUpload} disabled={!file || uploading}
            style={{ padding: '6px 14px', fontSize: '0.75rem', background: file ? '#c49ea4' : '#2d2d44', border: 'none', borderRadius: '6px', color: file ? '#151824' : '#5a4a5a', cursor: file ? 'pointer' : 'default' }}>
