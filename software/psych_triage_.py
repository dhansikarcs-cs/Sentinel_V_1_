...
try:
    from ai_kernel_ import assess_crisis_risk, get_emotion_labels
except Exception:
    assess_crisis_risk = get_emotion_labels = None
...
def _compute_priority(patient: str, crisis_state: dict) -> dict:
    ...
    last_raw = entries[-1].get("raw_content", "") if entries else ""
    silent = False
    if last_ts:
        try:
...
def render_psych_triage(username: str):
    ...
    for p in priorities:
        ...
        with st.expander(f"{_crisis_icon if is_crisis else ''}{pname} (@{patient})", expanded=is_crisis):
            ...
            if ps:
                _latest = ps[-1]
                _src = _latest.get("ai_source", "") or ""
                _emo = _latest.get("emotions", "") or ""
                _badge = ""
                if _src:
                    _sc = {"ollama": "#c49ea4", "groq": "#22c55e", "rule": "#f59e0b"}.get(_src, "#888")
                    _badge += f"<span style='background:{_sc}22;color:{_sc};font-size:0.6rem;padding:1px 6px;border-radius:3px;font-weight:600;border:1px solid {_sc}44;margin-left:6px;'>{_src.title()}</span>"
                st.markdown(
                    f"<div style='display:flex;align-items:center;gap:6px;'>"
                    f"<span style='font-weight:600;font-size:0.8125rem;'>AI Clinical Insight</span>{_badge}"
                    f"</div>",
                    unsafe_allow_html=True,
                )
                ...
                if _emo:
                    st.markdown(
                        f"<div style='color:#7a8aaa;font-size:0.65rem;margin-top:2px;'>Detected: {_emo}</div>",
                        unsafe_allow_html=True,
                    )
            else:
            ...
            _why_key = f"why_{patient}"
            if st.button("\U0001f50d Why this summary?", key=_why_key, use_container_width=True):
                st.session_state[_why_key + "_open"] = not st.session_state.get(_why_key + "_open", False)
            if st.session_state.get(_why_key + "_open", False):
                _why_src = _latest.get("ai_source", "") if ps else ""
                _why_emo = _latest.get("emotions", "") if ps else ""
                _why_mode = "clinical" if _latest.get("summary", "") else "patient"
                st.markdown(
                    f"<div style='background:#1a1e30;border:1px solid #2d2d44;border-radius:8px;padding:10px;margin:4px 0 10px 0;'>"
                    f"<div style='color:#c49ea4;font-size:0.7rem;font-weight:600;margin-bottom:4px;'>Explainability</div>"
                    f"<div style='color:#9aa8c0;font-size:0.6875rem;line-height:1.6;'>"
                    f"AI Source: <strong>{_why_src.title() if _why_src else 'N/A'}</strong><br>"
                    f"Prompt Mode: <strong>Clinical Summarization</strong><br>"
                    f"{'Detected Emotions: <strong>' + _why_emo + '</strong><br>' if _why_emo else ''}"
                    f"The summary was generated by Sentinel using a {_why_src.title() if _why_src else 'rule-based'} "
                    f"inference with a clinical documentation prompt. "
                    f"No raw journal text is exposed to preserve patient privacy."
                    f"</div></div>",
                    unsafe_allow_html=True,
                )

            _crisis_key = f"crisis_risk_{patient}"
            if p.get("last_raw"):
                if st.session_state.get(_crisis_key):
                    _cr = st.session_state[_crisis_key]
                    _cr_color = "#ef4444" if _cr.get("triggered") else "#22c55e"
                    _factors = _cr.get("contributing_factors", {})
                    _factor_html = ""
                    if _factors:
                        _check = "\u2713" if _factors.get("crisis_keywords") else ""
                        _high = "\u2713" if _factors.get("high_risk_keywords") else ""
                        _mod = "\u2713" if _factors.get("moderate_keywords") else ""
                        _soc = _factors.get("social_withdrawal", 0)
                        _slp = _factors.get("sleep_disturbance", 0)
                        _act = _factors.get("activity_decline", 0)
                        _soc_b = "rgba(239,68,68,0.3)" if _soc >= 2 else ("rgba(245,158,11,0.2)" if _soc >= 1 else "rgba(26,34,56,0.6)")
                        _slp_b = "rgba(239,68,68,0.3)" if _slp >= 2 else ("rgba(245,158,11,0.2)" if _slp >= 1 else "rgba(26,34,56,0.6)")
                        _act_b = "rgba(239,68,68,0.3)" if _act >= 2 else ("rgba(245,158,11,0.2)" if _act >= 1 else "rgba(26,34,56,0.6)")
                        _factor_html = (
                            f"<div style='display:flex;flex-wrap:wrap;gap:4px;margin-top:6px;'>"
                            f"<div style='background:{'rgba(239,68,68,0.2)' if _check else 'rgba(26,34,56,0.6)'};color:{'#ef4444' if _check else '#5a6a7a'};font-size:0.6rem;padding:2px 6px;border-radius:4px;'>{'&#9888;' if _check else ''} Crisis KW</div>"
                            f"<div style='background:{'rgba(239,68,68,0.2)' if _high else 'rgba(26,34,56,0.6)'};color:{'#ef4444' if _high else '#5a6a7a'};font-size:0.6rem;padding:2px 6px;border-radius:4px;'>{'&#9888;' if _high else ''} High Risk</div>"
                            f"<div style='background:{'rgba(245,158,11,0.2)' if _mod else 'rgba(26,34,56,0.6)'};color:{'#f59e0b' if _mod else '#5a6a7a'};font-size:0.6rem;padding:2px 6px;border-radius:4px;'>{'&#9898;' if _mod else ''} Moderate</div>"
                            f"<div style='background:{_soc_b};color:{'#ef4444' if _soc >= 2 else '#f59e0b' if _soc >= 1 else '#5a6a7a'};font-size:0.6rem;padding:2px 6px;border-radius:4px;'>Social {_soc}x</div>"
                            f"<div style='background:{_slp_b};color:{'#ef4444' if _slp >= 2 else '#f59e0b' if _slp >= 1 else '#5a6a7a'};font-size:0.6rem;padding:2px 6px;border-radius:4px;'>Sleep {_slp}x</div>"
                            f"<div style='background:{_act_b};color:{'#ef4444' if _act >= 2 else '#f59e0b' if _act >= 1 else '#5a6a7a'};font-size:0.6rem;padding:2px 6px;border-radius:4px;'>Activity {_act}x</div>"
                            f"</div>"
                        )
                    st.markdown(
                        f"<div style='background:{_cr_color}15;border:1px solid {_cr_color}44;border-radius:8px;padding:10px;margin:4px 0 10px 0;'>"
                        f"<div style='display:flex;justify-content:space-between;align-items:center;margin-bottom:4px;'>"
                        f"<div style='color:{_cr_color};font-size:0.7rem;font-weight:600;'>Crisis Risk Assessment</div>"
                        f"<div style='color:{_cr_color};font-size:1rem;font-weight:700;'>{_cr.get('risk_score', '?')}/10</div>"
                        f"</div>"
                        f"<div style='color:#9aa8c0;font-size:0.6875rem;line-height:1.6;'>{_cr.get('reasoning', 'No reasoning available.')}</div>"
                        f"{_factor_html}"
                        f"</div>",
                        unsafe_allow_html=True,
                    )
                else:
                    if st.button("\u26a0 Assess Crisis Risk", key=f"{_crisis_key}_btn", use_container_width=True):
                        _cr = safe(assess_crisis_risk, {"risk_score": 1, "reasoning": "Assessment unavailable.", "triggered": False}, p["last_raw"])
                        st.session_state[_crisis_key] = _cr
                        st.rerun()
